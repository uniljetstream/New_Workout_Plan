...
void MainWindow::sendPoseIndex(int poseIndex)
{
    ...
    // 조이스틱 진동 명령 전송 (pose index가 변경될 때마다 1초 진동)
    QJsonObject vibrationJson;
    vibrationJson["vibration"] = "ON";
    vibrationJson["time"] = 1.0;  // 1초

    QJsonDocument vibrationDoc(vibrationJson);
    publishMessage(m_config.topicWatchtowerCmdJoystick(), vibrationDoc.toJson(QJsonDocument::Compact));
    qDebug() << "→ Sent vibration command to joystick (1.0s)";
}
...
void MainWindow::requestPoseAnalysis(int poseIndex)
{
    ...
    // sendPoseIndex는 포즈가 실제로 변경될 때만 호출되어야 함
    // 분석 요청 시마다 호출하지 않음
